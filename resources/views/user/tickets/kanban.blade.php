...
@section('title', 'Department Agile Board')

@section('content')
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
        <div>
            <h1 class="text-3xl font-extrabold text-base-content tracking-tight">Department Agile Board</h1>
            <p class="text-sm text-base-content/40 mt-1 font-medium italic">Drag and drop department tickets to update their status.</p>
        </div>
    </div>
        ...
        <div class="flex-1 min-w-[300px] bg-base-200/50 rounded-3xl p-4 flex flex-col gap-4 border border-base-content/5 shadow-sm">
            ...
            <div id="column-open" data-status="open" class="kanban-column flex flex-col gap-3 min-h-[150px]">
                @foreach($kanban['open'] as $ticket)
                    @include('user.tickets.partials.kanban-card', ['ticket' => $ticket])
                @endforeach
            </div>
        </div>
            ...
            columns.forEach(column => {
                new Sortable(column, {
                    ...
                    onEnd: function (evt) {
                        ...
                        fetch(`/user/tickets/${ticketId}/status`, {
                            method: 'PATCH',
                            headers: {
