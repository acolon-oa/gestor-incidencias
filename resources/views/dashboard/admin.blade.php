...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 mt-2">
        <div class="bg-base-100 border border-base-content/5 rounded-3xl p-8 shadow-sm hover:shadow-md transition-all group">
            <div class="flex items-start justify-between mb-4">
                ...
                <span class="text-[10px] font-black uppercase tracking-widest text-base-content/20">Open</span>
            </div>
            <div class="text-4xl font-black text-base-content tracking-tighter">{{ $openTicketsCount }}</div>
            <div class="text-xs font-bold text-base-content/40 mt-1">Global Open Tickets</div>
        </div>
        <div class="bg-base-100 border border-base-content/5 rounded-3xl p-8 shadow-sm hover:shadow-md transition-all group">
            <div class="flex items-start justify-between mb-4">
                ...
                <span class="text-[10px] font-black uppercase tracking-widest text-base-content/20">Pending</span>
            </div>
            <div class="text-4xl font-black text-base-content tracking-tighter">{{ $myPendingTicketsCount }}</div>
            <div class="text-xs font-bold text-base-content/40 mt-1">Pending My Action</div>
        </div>
        <div class="bg-base-100 border border-base-content/5 rounded-3xl p-8 shadow-sm hover:shadow-md transition-all group">
            <div class="flex items-start justify-between mb-4">
                ...
                <span class="text-[10px] font-black uppercase tracking-widest text-base-content/20">Resolved</span>
            </div>
            <div class="text-4xl font-black text-base-content tracking-tighter">{{ $resolvedTicketsCount }}</div>
            <div class="text-xs font-bold text-base-content/40 mt-1">Recently Resolved</div>
        </div>
    </div>
    ...
    <div class="bg-base-100 border border-base-content/5 rounded-3xl shadow-sm overflow-hidden">
        <div class="flex flex-col sm:flex-row items-center justify-between px-8 py-6 border-b border-base-content/5 gap-4">
            <div>
                <h2 class="text-xl font-black text-base-content tracking-tight">System Activity</h2>
                <p class="text-xs text-base-content/40 mt-0.5 font-medium italic">Latest incidents across the organization</p>
            </div>
            <form action="{{ route('admin.dashboard') }}" method="GET" class="flex items-center gap-2">
                <div class="join border border-base-content/10 rounded-2xl overflow-hidden shadow-sm bg-base-100">
                    <input type="text" name="ticket_id" value="{{ request('ticket_id') }}" placeholder="ID..." class="input input-sm join-item w-24 bg-transparent focus:outline-none text-xs font-bold" />
                    ...
                    <button type="submit" class="btn btn-sm btn-primary join-item px-6 font-bold shadow-sm shadow-primary/20">Search</button>
                </div>
                @if(request()->anyFilled(['ticket_id', 'status']))
                    <a href="{{ route('admin.dashboard') }}" class="btn btn-sm btn-ghost btn-circle">
        ...
        <div class="overflow-x-auto">
            <table class="table w-full">
                <thead>
                    <tr class="bg-base-200/50 text-base-content/30 border-b border-base-content/5">
                        <th class="font-black uppercase text-[10px] tracking-widest py-5 pl-8">ID</th>
                        <th class="font-black uppercase text-[10px] tracking-widest py-5">Incident Details</th>
                        ...
                        <th class="font-black uppercase text-[10px] tracking-widest py-5 pr-8 text-right">Last Update</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-base-content/5 font-medium">
                    @forelse($tickets as $ticket)
                    <tr class="hover:bg-primary/5 transition-colors cursor-pointer group" onclick="window.location='{{ route('admin.tickets.show', $ticket->id) }}'">
                    ...
                    <tr>
                        <td colspan="5" class="text-center py-20">
                            <div class="w-16 h-16 bg-base-200 rounded-3xl flex items-center justify-center mx-auto mb-4">
                                <x-heroicon-o-adjustments-horizontal class="w-8 h-8 text-base-content/20" />
                            </div>
                            <p class="text-base-content/40 font-bold italic">No incidents found matching criteria.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
